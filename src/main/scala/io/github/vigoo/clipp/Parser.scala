...
object Parser {
  ...
  private object impl {
    ...
    def simpleParameter[T](simpleParam: SimpleParameter[T]): ExtractStateM[T] = {
      for {
        state: ExtractParametersState <- getState
        firstMatch: Option[String] = state.nonParsedArguments.map(_.value).find(p => !p.startsWith("-"))
        result <- firstMatch match {
          case Some(value) =>
            val t1: ExtractStateM[T] = parseString(simpleParam.parameterParser, value)
            t1
          case None =>
            val t2: ExtractStateM[Nothing] = failWith(MissingSimpleParameter(simpleParam.placeholder))
            t2
        }
        _ <- setState(
          firstMatch match {
      ...
      for {
        ...
        result <- firstMatch match {
          case Some(positionedParameter) =>
            val p: ExtractStateM[PositionedParameter] =
              if validateCommandLocations then {
                state.remainingCommandPositions.headOption match {
                  case Some(CommandLocation(position, value))
                    ...
                    if cmd.validCommands.contains(positionedParameter.value) then {
            ...
            p
          case None =>
            val n: ExtractStateM[Nothing] = failWith(MissingCommand(cmd.validCommands))
            n
        }
        _ <- setState(
          firstMatch match {
